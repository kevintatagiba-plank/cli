...
export class ComputerTool implements BaseAnthropicTool {
  ...
  private async handleMouseAction(action: Action, coordinate: [number, number]): Promise<ToolResult> {
    ...
    if (action === Action.MOUSE_MOVE) {
      ...
      await this.kernel.browsers.computer.clickMouse(this.sessionId, {
        x, y, button: 'left', click_type: 'down',
      });
      this.lastMousePosition = [x, y];
    } else if (action === Action.LEFT_MOUSE_UP) {
      await this.kernel.browsers.computer.clickMouse(this.sessionId, {
        x, y, button: 'left', click_type: 'up',
      });
      this.lastMousePosition = [x, y];
    } else {
      const button = this.getMouseButton(action);
      let numClicks = 1;
      if (action === Action.DOUBLE_CLICK) numClicks = 2;
      else if (action === Action.TRIPLE_CLICK) numClicks = 3;

      await this.kernel.browsers.computer.clickMouse(this.sessionId, {
        x, y, button, click_type: 'click', num_clicks: numClicks,
      });
      this.lastMousePosition = [x, y];
    }
  ...
  private static readonly KEY_MAP: Record<string, string> = {
    'return': 'Return', 'enter': 'Return', 'Enter': 'Return',
    'left': 'Left', 'right': 'Right', 'up': 'Up', 'down': 'Down',
    'ArrowLeft': 'Left', 'ArrowRight': 'Right', 'ArrowUp': 'Up', 'ArrowDown': 'Down',
    'home': 'Home', 'end': 'End',
    'pageup': 'Page_Up', 'page_up': 'Page_Up', 'PageUp': 'Page_Up',
    'pagedown': 'Page_Down', 'page_down': 'Page_Down', 'PageDown': 'Page_Down',
    'delete': 'Delete', 'backspace': 'BackSpace', 'Backspace': 'BackSpace',
    'tab': 'Tab', 'insert': 'Insert',
    'esc': 'Escape', 'escape': 'Escape',
    'f1': 'F1', 'f2': 'F2', 'f3': 'F3', 'f4': 'F4', 'f5': 'F5', 'f6': 'F6',
    'f7': 'F7', 'f8': 'F8', 'f9': 'F9', 'f10': 'F10', 'f11': 'F11', 'f12': 'F12',
    'space': 'space', 'minus': 'minus', 'equal': 'equal', 'plus': 'plus',
  };
  ...
  private static readonly MODIFIER_MAP: Record<string, string> = {
    ...
    'meta': 'super', 'Meta': 'super', 'cmd': 'super', 'command': 'super', 'win': 'super', 'super': 'super',
  };

  private convertToKernelKey(key: string): string {
    if (ComputerTool.MODIFIER_MAP[key]) return ComputerTool.MODIFIER_MAP[key];
    if (ComputerTool.KEY_MAP[key]) return ComputerTool.KEY_MAP[key];
    return key;
  }
  ...
  async call(params: ActionParams): Promise<ToolResult> {
    const { action, text, coordinate, scrollDirection: scrollDirectionParam, scroll_amount, scrollAmount, duration, ...kwargs } = params;
    ...
    if (action === Action.SCROLL) {
      ...
      const [x, y] = coordinate ? ActionValidator.validateAndGetCoordinates(coordinate) : this.lastMousePosition;
      const scrollDelta = ((scrollAmountValue as number) ?? 1) * 120;

      let delta_x = 0, delta_y = 0;
      if (scrollDirection === 'down') delta_y = scrollDelta;
      else if (scrollDirection === 'up') delta_y = -scrollDelta;
      else if (scrollDirection === 'right') delta_x = scrollDelta;
      else if (scrollDirection === 'left') delta_x = -scrollDelta;
    ...
    if (action === Action.LEFT_CLICK_DRAG) {
      if (!coordinate) throw new ToolError(`coordinate is required for ${action}`);
      ...
      const [startX, startY] = startCoordinate ? ActionValidator.validateAndGetCoordinates(startCoordinate) : this.lastMousePosition;
    ...
    if (this.keyboardActions.has(action)) {
      if (!text) throw new ToolError(`text is required for ${action}`);
